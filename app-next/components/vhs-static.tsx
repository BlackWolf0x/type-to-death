interface VHSStaticProps {
    opacity?: number;
    className?: string;
}

export function VHSStatic({ opacity = 0.7, className = '' }: VHSStaticProps) {
    return (
        <div
            className={`pointer-events-none fixed inset-0 z-0 mix-blend-multiply ${className} animate-pulse`}
            style={{
                opacity,
