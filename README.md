...
├── highscores.ts       # High score queries and mutations
├── http.ts             # HTTP endpoints
├── schema.ts           # Database schema
├── stories.ts          # Story generation functions
...
RECRAFT_API_KEY         # Recraft.ai API key for image generation
SITE_URL                # Your site URL (e.g., https://yourdomain.com)
CLAUDE_API_KEY          # Anthropic Claude API key for story generation
TINIFY_API_KEY          # TinyPNG API key for image compression
```
...
Your `.env.local` file (auto-generated by `npx convex dev`):
